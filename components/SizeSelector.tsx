...
const SizeSelector: React.FC<SizeSelectorProps> = ({ variants, selectedVariant, setSelectedVariant }) => {
  ...
  return (
    <SizeSelectorContainer>
      {variants?.map((variant, index) => (
        <SizeSelectorWrapper className={`${variant.stock === 0 ? 'disabled' : ''}`} selected={variant.size === selectedVariant?.size} key={index}>
          <span onClick={() => onSetSelectedVariant(variant)}>{variant.size}</span>
        </SizeSelectorWrapper>
      ))}
    </SizeSelectorContainer>
  );
